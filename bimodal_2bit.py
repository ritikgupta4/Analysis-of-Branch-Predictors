...
for i in range(table_size):
    bht_table.append(3)         #initialising or predicting as ST for the very first iteration
...
for line in lines:
    ...
    #print(sline[0]%16)
    if(0<bht_table[address % table_size] and bht_table[address % table_size]<=3 and sline[1]=="NT"):
        if (bht_table[address % table_size]<2):
            correct=correct+1
        bht_table[address % table_size]=bht_table[address % table_size]-1
    elif(bht_table[address % table_size]>=0 and bht_table[address % table_size]<3 and sline[1]=="T"):
        if(bht_table[address % table_size]>=2):
            correct=correct+1
        bht_table[address % table_size]=bht_table[address % table_size]+1
    else:
